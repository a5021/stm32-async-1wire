...
| `demo6` | `src/demo6.c`    | Low-power example (same search + sequential loop as `demo1`): with `-DOW_PORT_LOW_POWER` the main loop blocks in `__WFE()` while a long 1-Wire stage (> 1 ms: temperature conversion, scratchpad read, EEPROM hold-off, inter-cycle pause) is running, instead of busy-polling. Without the define it compiles to a plain busy-poll loop. |
...
| `EXT="-DOW_PORT_LOW_POWER"` | Enable the opt-in low-power path: TIM1 UIE + `SEVONPEND` so the application can `__WFE()`-sleep during long 1-Wire stages (> 1 ms) instead of busy-polling (see demo6). The driver itself stays non-blocking; no ISR is installed. Without this define builds are byte-identical to the original. |
